...
class pl1hp:
    ...
    def draw(self):
        self.image.clip_draw(0,0,self.hp,25,self.hpx,500)
        self.image2.draw(210,500,250,25)
    def update(self):
        if pl1.status==4:
            if pl2.status2==2:
                self.hpx+=5
...
class pl2hp:
    def __init__(self):
        ...
        self.image2 = load_image('hpg.png')
    def draw(self):
        self.image.clip_draw(0,0,self.hp2,25,self.hpx2,500)
        self.image2.draw(590,500,250,25)
    def update(self):
        if pl2.status2==4:
            if pl1.status==2:
                self.hpx2-=5
...
class player1:
    ...
    def get_bb(self):
        if self.status ==7 and self.frame>=8:
            return self.x-20, self.y-50, self.x+200, self.y+50
        else:
...
def update():
    if pl1.status==5 and pl1.frame>7:
        pl1.win.draw(300,400)
